#!/usr/bin/env/python

#######################################################
# Takes the file generated by get_tweets.py           #
# and assigns each twitter handle a numeric value and #
# sorts all tweet events by their time                #
#######################################################

import argparse
import common

def main(args):
    a = common.UID_Assigner(args.leader)
    tweet_events = []

    with open(args.tweets_file, 'r') as tweets_file:
        with open(args.processed_file, 'wb') as output:
            for line in tweets_file:
                line = line.strip('\n')
                line = line.split(",") 

                # reduce usernames to node numbers (1-indexed, and 0 indicates NULL)
                user = a.get_UID(line[1])
                retweeted = a.get_UID(line[2])

                # keep track of the triple: (timestamp, user's id, id of who they retweeted)
                tweet_events.append((long(line[0]), str(user), str(retweeted)))
                
            # sort the events by timestamp
            tweet_events = sorted(tweet_events, key = lambda (timestamp, fid, cid): timestamp)
            # write down the triples in order
            for tweet in tweet_events:
                output.write(str(tweet[0]) + "," + tweet[1] + "," + tweet[2]+"\n")

# python postprocessing.py --leader <handle of broadcaster> --tweets_file PATH/TO/tweets/<handle>.txt --processed_file PATH/TO/tweets_processed/<handle>.txt
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Specify arguments')
    parser.add_argument('--leader', help='handle of the leader node', required=True)
    parser.add_argument('--tweets_file', help='the tweets file produced by get_tweets.py', required=True)
    parser.add_argument('--processed_file', help='the output file', required=True)

    args = parser.parse_args()
    main(args)